...
<div id="DPD_pvz_popup" style="display: none;">
	<div id="DPD_pvz" data-component-params='<?= json_encode($arParams) ?>' data-component-template="<?= $templateName ?>">
		<div id="DPD_map">
			<? $APPLICATION->IncludeComponent('bitrix:map.yandex.view', 'dpd.pickup', array(
				'INIT_MAP_TYPE' => 'MAP',
				'MAP_WIDTH'     => '100%',
				'MAP_HEIGHT'    => '500',
				'CONTROLS'      => array('ZOOM'),
				...
				<div id="DPD_wrapper">
					<?php if ($arResult['TERMINALS']) { ?>
						<?php foreach ($arResult['TERMINALS'] as $arTerminal) { ?>
							<p class="DPD_terminalSelect" data-terminal-code="<?= $arTerminal['CODE'] ?>" data-terminal-address="<?= htmlspecialchars($arTerminal['ADDRESS_FULL']) ?>"><?= $arTerminal['NAME'] ?></p>
						<?php }?>
					<?php } ?>
				</div>
			</div>
