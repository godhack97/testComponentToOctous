...
class IpolhDpdPickupComponent extends \CBitrixComponent
{
	...
	public function processAjaxRequest()
	{
        ...
        $data = $this->$method();
        $data = \Ipolh\DPD\Utils::convertEncoding($data, SITE_CHARSET, 'UTF-8');
	...
	public function ajaxGetTerminalData()
	{
		$arItem = \Ipolh\DPD\DB\Terminal\Model::getByCode($_REQUEST['terminal_code']);
	...
	public function getTerminals()
	{
		...
		$pvz_type = Option::get(IPOLH_DPD_MODULE, "TYPE_DISPLAYED_PVZ", array());
        $pvz_type = unserialize ($pvz_type);
        if (empty($pvz_type[0])){
            $pvz_type = array('П','ПВП');
        }
        ...
        $this->arResult['TERMINALS'] = \Ipolh\DPD\DB\Terminal\Model::getList([
			'select' => ['*'],
			'filter' => array_merge(
				['SHIPMENT' => $shipment],
        ...
        $this->arResult['TERMINALS'] = $this->onPVZListReady($this->arResult['TERMINALS']);
        return $this->arResult['TERMINALS'];
	}
	...
	public function onPVZListReady($terminals){
        $event = new \Bitrix\Main\Event("ipol.dpd", "onPVZListReady", array('TERMINALS'=>$terminals));
        $event->send();
        ...
        foreach ($event->getResults() as $eventResult){
            ...
            $terminals = $event->getParameters('TERMINALS')['TERMINALS'];
        }
        return $terminals;
    }
