<?php
    ...
    Page::getInstance()->setTitle(getMessageModule('XwildRestMenuItemJournalRequestLimit'));
    $sTableID = 'tbl_'.Settings::getInstance()->getTable()['request-limit'];
    ...
    if ($ids = $lAdmin->GroupAction()) {
        ...
        if ($ids) {
            switch ($_REQUEST['action']) {
                case 'delete':
                    Journal::getInstance()->deleteData('request-limit', $ids);
                    break;
            }
        }
    }
    ...
    $arFilterFields = [
        'find_id', 'find_date_from', 'find_date_to', 'find_client_id'
    ];
    $arFilter = [];
    ...
    $rsData = Journal::getInstance()->getData('request-limit', $arFilter, $arSort);
    ...
    $arFilterNames = [
        ...
        'find_client_id' => getMessageModule('XwildRestClientId')
    ];
