...
class ArlixStorePage extends CBitrixComponent implements Controllerable
{
    ...
    public function getelementAction($id = 0)
    {
        ...
        $res = \CIBlockElement::GetList([], $arFilter, false, ["nPageSize" => 1], ["ID", "IBLOCK_ID", "NAME", "PROPERTY_CML2_ARTICLE", "DETAIL_PAGE_URL"]);
        if ($ob = $res->GetNext())
        {
            $result['id'] = (int)$ob["ID"];
            $result['name'] = $ob["NAME"];
            $result['url'] = $ob["DETAIL_PAGE_URL"];
            $result['prop'][] = [
                'name' => "Артикул",
