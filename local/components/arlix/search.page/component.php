<?php
  ...
  if (isset($_REQUEST["q"])) {
    $prepareQuery = $_REQUEST['q'];
    $exceptionList = ['юбка', 'блузка', 'олимпийка', 'заколка', 'рубашка', 'гвоздика', 'футболка', 'маска', 'майка', 'водолазка'];

    if(preg_match('/ка$/', $_REQUEST['q']) && !in_array(mb_strtolower($_REQUEST['q']), $exceptionList)){
      $prepareQuery = preg_split('/ка$/', $_REQUEST['q'])[0];
    }
    $q = trim($prepareQuery);
    //$q = trim($_REQUEST["q"]);
  }
  ...
  if ($arParams["SHOW_WHEN"] && isset($_REQUEST["from"]) && is_string($_REQUEST["from"]) && mb_strlen($_REQUEST["from"]) && CheckDateTime($_REQUEST["from"])) {
    $from = $_REQUEST["from"];
  }
  else {
    $from = "";
  ...
  if ($arParams["SHOW_WHEN"] && isset($_REQUEST["to"]) && is_string($_REQUEST["to"]) && mb_strlen($_REQUEST["to"]) && CheckDateTime($_REQUEST["to"])) {
    $to = $_REQUEST["to"];
  }
  else {
    $to = "";
