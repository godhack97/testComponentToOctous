...
$isManager = CSite::InGroup(explode(',', Option::get('sodamoda.core', 'group_manage_lists')));
...
<section class="section-editing">
    <div class="heading">
        ...
        ?>
            <div class="buttons">
                <a class="black hover" href="<?= $arParams['PATH_ADD'] ?>" target="_blank">Добавить товар</a>
                <a class="black hover delete-product">Удалить товар</a>
            </div>
        <?php
        }
        ?>
    </div>
    <form method="post" name="save_product" class="props-form form js-props-form" enctype="multipart/form-data" <?= $isActionEdit ? ' data-product="' . $editProduct . '"' : ''; ?> data-type-form='<?= $arParams['ACTION'] ?>' data-return="<?= $arParams['PATH_RETURN']; ?>" data-admin="<?= $arParams['IS_ADMIN'] ? 'true' : 'false'; ?>">
        <?php
        if ($isActionEdit) {
        ?>
        ...
        ?>
        <div class="field field-checkbox field-activity">
            <label>
        ...
        <div class="field field-checkbox field-activity">
            <?php if ($arResult["MODERATION"] && $isManager) { ?>
                <label>
                    <input type="checkbox" id="moderate" name="moderate" value="Y">
                    <div class="label">Снять с модерации</div>
                </label>
            <? } ?>
            <?php if ($arResult["MODERATION"] && !$isManager) { ?>
                <div class="">На модерации</div>
            <? } ?>
        </div>
        ...
        <div class="props-wrapper">
            ...
            <div class="props-content ru-content">
                <div class="props-item">
                    ...
                    <div class="props-element">
                        <div class="field">
                            <input class="field-style" type="text" placeholder=" " name="article" id="article" maxlength="" value="<?= !$isActionEdit ? '' : $arResult['ARTICLE'] ?>" <?= !$isActionEdit ?: 'readonly' ?> required>
                ...
                <div class="props-item">
                    <div class="props-element">
                        <div class="field">
                            <input class="field-style" type="number" placeholder=" " name="price" id="price" min="0" maxlength="" step="0.01" value="<?= !$isActionEdit ? '' : $arResult['PRICE_RUB'] ?>" required>
                            <label class="placeholder" for="price">Цена RUB</label>
                        </div>
                    </div>
                    <div class="props-element select-field">
                        <select class="new-select" id="section_id" name="section[]" multiple="multiple" required>
                            <option value='0'>—</option>
                            <?php
                            foreach ($arResult['SECTIONS'] as $sectionID => $sectionName) {
                            ?>
                                <option <?= (in_array($sectionID, $arResult['SECTION']) ? 'selected ' : '') ?>value="<?= $sectionID ?>"><?= $sectionName ?></option>
                            <?php
                            }
                            ?>
                        </select>
                    ...
                    <div class="props-item props-add">
                        <div class="props-element select-field">
                            <select name="material-id[]" class="new-select">
                                ...
                                ?>
                                    <option value="<?= $eMatID ?>"><?= ((SITE_ID == 's1') ? $eMat['NAME'] : $eMat['NAME_EN']) ?></option>
                                <?php
                                }
                                ?>
                            </select>
                        ...
                        <div class="remove-button js-remove-material" title="Удалить материал"></div>
                    </div>
                    <?
                } else {
                    $fflag = true;
                        ...
                        <div class="props-item <?= (($fflag) ? 'props-add' : '') ?>">
                            <div class="props-element select-field">
                                <select name="material-id[]" class="new-select">
                                    ...
                                    foreach ($arResult['ENUM']['MATERIALS'] as $eMatID => $eMat) {
                                    ?>
                                        <option <?= (($eMatID == $materialID) ? 'selected' : '') ?> value="<?= $eMatID ?>"><?= ((SITE_ID == 's1') ? $eMat['NAME'] : $eMat['NAME_EN']) ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
            ...
            <div class="props-content en-content">
                ...
                <div class="field">
                    <input class="field-style" type="text" placeholder=" " name="price_en" id="price_en" maxlength="" min="0" value="<?= !$isActionEdit ? '' : $arResult['PRICE_USD'] ?>" required>
                    ...
                    <div class="color-element props-item">
                        <div class='select-field'>
                            <select id='sel-0' class='new-select color-select'>
                                ...
                                ?>
                                    <option <?= (($fflag) ? 'selected' : '') ?> value="<?= $colCode ?>"><?= ((SITE_ID == 's1') ? $arColor['NAME'] : $arColor['NAME_EN']) ?></option>
                                <?php
                                    if ($fflag) {
                            ...
                            <div class="remove-button js-remove-color" title="Удалить цвет"></div>
                        </div>
                    ...
                    <div class="sizes-element props-item">
                        <div class="props-element select-field">
                            <select id='selsize-0' class="new-select size-select" multiple="multiple" name="sizes[]">
                                ...
                                ?>
                                    <option <?= ((in_array($sizeVal, $arResult['SIZES'])) ? 'selected ' : '') ?>value="<?= $sizeID ?>"><?= $sizeVal ?></option>
                                <?php
                                }
                                ?>
                            </select>
                        ...
                        <div class='color-element props-item'>
                            <div class='select-field'>
                                <select id="sel-<?= $i ?>" class='new-select color-select'>
                                    ...
                                    foreach ($arResult['ENUM']['COLORS'] as $colCode => $arColor) {
                                    ?>
                                        <option <?= (($colCode == $colorCode) ? 'selected ' : '') ?>value="<?= $colCode ?>"><?= ((SITE_ID == 's1') ? $arColor['NAME'] : $arColor['NAME_EN']) ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
                            ...
                            <div class='images'>
                                ...
                                ?>
                                    <a class='img' data-fancybox='gallery' href="<?= $arPhoto['PATH'] ?>">
                                        <input type="hidden" class="file-<?= $i ?>" name="<?= $colorCode ?>[]" value="<?= $arPhoto['CODE'] ?>">
        ...
        <div class="buttons-wrapper props-content">
            <?php
            if ($isActionEdit) {
            ?>
                <div class="props-item">
                    <div class="props-element">
                        <a href="<?= $arParams['PATH_RETURN']; ?>" class="ibutton hollow cancel-product" type="button">Назад в список</a>
                    </div>
                    <div class="props-element">
