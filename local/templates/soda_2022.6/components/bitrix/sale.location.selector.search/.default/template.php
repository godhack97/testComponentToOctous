...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <?if(strlen($arResult['MODE_CLASSES'])):?> <?=$arResult['MODE_CLASSES']?><?endif?>">
		...
		<div class="<?=$dropDownBlock?> field field-city bx-ui-sls-input-block">

			<span class="dropdown-icon"></span>
			<input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" value="<?=$arResult['VALUE']?>" class="dropdown-field field-style" placeholder="<?=Loc::getMessage('SALE_SLS_INPUT_SOME')?>" />

			<div class="dropdown-fade2white"></div>
			<div class="bx-ui-sls-loader"></div>
      <!-- div class="bx-ui-sls-clear" title="<?=Loc::getMessage('SALE_SLS_CLEAR_SELECTION')?>"><img class="icon-clear" src="<?=SITE_TEMPLATE_PATH?>/images/icons/edit.svg"></div -->
			<div class="bx-ui-sls-pane"></div>

      <label class="placeholder" for=""><?=Loc::getMessage('SALE_SLS_CITY')?></label>
		</div>
