...
//      'arrFILTER' => array('iblock_' . $arParams['IBLOCK_TYPE'], 'main'),
...
// Если элементы найдены - фильтр по ним
if(!empty($arElements) && is_array($arElements))
	$searchFilter = ["=ID" => $arElements];
else
{
	// Иначе по запросу (в arResult они в protected)
	$q = htmlspecialcharsbx($_GET['q']);
	
	// Парсим
	if(preg_match('/^([0-9]+)|([0-9]+-[0-9]+)$/', $q))
		$searchFilter = ['PROPERTY_CML2_ARTICLE' => '%'.$q.'%'];
	else
		$flag = false;
}
