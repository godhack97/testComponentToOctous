...
<?if($arParams["~FOR_SPLIT"] == 'Y'):?>
  <div class="socials">
    <?foreach($arAuthServices as $service):?>
      ...
      if(htmlspecialcharsbx($service["ICON"]) == 'facebook')
        $service["ICON"] = 'fb';
      
      if(htmlspecialcharsbx($service["ICON"]) == 'vkontakte')
        $service["ICON"] = 'vk';
      
      if(htmlspecialcharsbx($service["ICON"]) == 'instagram')
        $service["ICON"] = 'ins';
      
      ?>
      <a class="ibutton hollow soc-<?=htmlspecialcharsbx($service["ICON"])?>" title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"></a>
    <?endforeach?>
    <a class="ibutton hollow soc-phone"><?=Loc::getMessage("SOCSERV_AUTH_PHONE")?></a>
  </div>
...
<div class="bx-auth">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		<?if($arParams["~SHOW_TITLES"] != 'N'):?>
			<div class="bx-auth-title"><?=Loc::getMessage("socserv_as_user")?></div>
			<div class="bx-auth-note"><?=Loc::getMessage("socserv_as_user_note")?></div>
		<?endif;?>
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
