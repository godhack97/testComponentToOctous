<? if (!defined('B_PROLOG_INCLUDED') || B_PROLOG_INCLUDED !== true) die();
  ...
  elseif ($cache->startDataCache() && !isset($_GET['q']) && (!isset($_GET['set_filter']) && !isset($_GET['del_filter'])) && $APPLICATION->GetCurDir() == '/catalog/sale/') {
    ...
    */
	      ...
	      if(isset(${$arParams["FILTER_NAME"]}['OFFERS'])){
            ...
            switch ($kFilterOffers){
              case '=PROPERTY_72':
                ...
                if(count($arItemsNew[$offer['ID']]['colors']) < 1){
                  //unset($arItemsNew[$arItem['ID']]);
                }
                break;
						}
    ...
    $newListProduct = [];
    foreach ($arItemsNew as $itemKey => $data){
      foreach ($data['colors'] as $kColor => $color) {
        $itemColorID = false;
        //$colors = $data['colors'];
        $colors = [];
        foreach ($data['colors'] as $iColor => $vColor){
          if($color['item']['ID'] === $vColor['item']['ID']){
            $colors[$iColor] = $vColor;
            //$itemColorID = $iColor;
          }
        }
        
        $newListProduct[$color['item']['ID']]['colors'] = $colors;
      }
    }
    foreach ($arItemsNew as $itemKey => $data){
      foreach ($data['colors'] as $kColor => $color) {
        ...
        //$colors = $data['colors'];
        $colors = [];
        $newListProduct[$color['item']['ID']]['colors'] = array_merge($newListProduct[$color['item']['ID']]['colors'], $data['colors']);
      }
    }
    
    $arItemsNew = $newListProduct;
		foreach ($arItemsNew as $kItem => $vItem){
			if(isset($vItem['colors'])){
				foreach ($vItem['colors'] as $color){
					if(isset($color['item']) && $color['item']['ID'] === $kItem){
						$sort = 0;
						if($APPLICATION->GetCurDir() == '/catalog/new/' || $APPLICATION->GetCurDir() == '/en/catalog/new/'){
							if($sort < $color['item']['SORT_NEW']){
								$sort = $color['item']['SORT_NEW'];
