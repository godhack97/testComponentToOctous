<? if (!defined('B_PROLOG_INCLUDED') || B_PROLOG_INCLUDED !== true) die();
  ...
  $jsObject = [];
  $daysLabelNewSeason = strtotime("-90 days");
  ...
  if (isset($item['colors'])) {
    ...
    if (strtotime($item['DATE_CREATE']) > $daysLabelNewSeason) {
      $subLabelName = Loc::getMessage('ITEM_NEW_SEASON');
    } else {
      $subLabelName = $item['NAMES'][ LANGUAGE_ID ];
      if(empty($subLabelName))
        $subLabelName = '&nbsp';
    }
    ...
    foreach ($item['colors'] as $kColor => $vColor) {
      ...
      $itemColorData = [
        ...
        'sort' => $vColor['item']['SORT'],
        'article' => $vColor['item']['ARTICLE'],
        'is_full' => $vColor['item']['PROPERTIES']['FULL']['VALUE_XML_ID'] == 'Y',
        'is_new_season' => strtotime($item['DATE_CREATE']) > $daysLabelNewSeason,
        'new_season_text' => $subLabelName,
...
<div class="item-wrapper">
  ...
  <?if($currentItem['exist_detail_picture']){?>
    <div class="images-slider">
      <div class="images-wrapper-new<?=$currentItem['exist_video'] ? '' : ' active';?>" id="image_block_<?=$currentItem['id'];?>">
        <?if (!empty($currentItem['more_photo'])):?>
          <?foreach ($currentItem['more_photo'] as $key => $imageID):?>
            ...
            <?if ($key == 0) {?>
              <div class="image-container skeleton image-container__first">
                <div class="image">
      ...
      <a href="<?=$currentItem['url'];?>" class="images-mouse-tabs">
        <?if (count($currentItem['more_photo']) > 1): ?>
          <? for ($i = 0; $i < count($currentItem['more_photo']); $i++) {?>
            <div class="images-mouse-tab" data-index-tab="<?=$i?>"></div>
          <?}?>
        <?endif;?>
      </a>
      <div class="images-mouse-lines">
        <?if (count($currentItem['more_photo']) > 1): ?>
          <? for ($i = 0; $i < count($currentItem['more_photo']); $i++) {?>
            <div class="images-mouse-line" data-index-line="<?=$i?>"></div>
          <?}?>
        <?endif;?>
      </div>
    </div>
  ...
  <?}?>
	...
	<span class="subtitle-name<?if(!empty($currentItem['new_season_text'])):?> skeleton<?else:?> empty<?endif;?>" id="new_season_block_<?=$currentItem['id'];?>">
    <?= $currentItem['new_season_text'] ?>
    
		<?if($isRuleModerator):?>
      <? $strModerDataName = Loc::getMessage("CATALOG_ITEM_SORT") .' '. $currentItem['sort'] ."&nbsp;&nbsp;&nbsp;". Loc::getMessage("CATALOG_ITEM_ARTICUL") .' '. $currentItem['article']; ?>
			<?if(in_array($item['PRODUCT']['TYPE'], [1, 3])):?>
				<a class="other-info" href="/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=<?=CATALOG_ID?>&type=catalogs&ID=<?=$currentItem['id'];?>&lang=ru" target="_blank" id="manager_info_<?=$currentItem['id'];?>" data-name-sort="<?=Loc::getMessage("CATALOG_ITEM_SORT")?>" data-name-article="<?=Loc::getMessage("CATALOG_ITEM_ARTICUL")?>"><?=$strModerDataName?></a>
			<?else:?>
				<a class="other-info" href="/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=<?=CATALOG_ID?>&type=catalogs&ID=<?=$arResult['ITEM']['LINK_ELEMENT_ID']?>&lang=ru" target="_blank" id="manager_info_<?=$currentItem['id'];?>" data-name-sort="<?=Loc::getMessage("CATALOG_ITEM_SORT")?>" data-name-article="<?=Loc::getMessage("CATALOG_ITEM_ARTICUL")?>"><?=$strModerDataName?></a>
			<?endif?>
		<?endif?>
