...
BX.ready(function() {
	if(localStorage.getItem('referrerCatalog')){
		jQuery('.ibutton.hollow').attr('href', localStorage.getItem('referrerCatalog'));
	}
   ...
   * Поделиться корзиной
   */
  $(document).on('click', '.cart_sharing', function() {
  ...
  var executeCopy = function(val) 
  {
    ...
    alert(BX.message('SALE_BASKET_COPY_LINK'));
  };


  async function cartShare(link) {
    ...
    var title = 'SODAMODA Корзина покупок'; //title_input.disabled ? undefined : title_input.value;
    var text = 'SODAMODA Корзина покупок'; //text_input.disabled ? undefined : text_input.value;
