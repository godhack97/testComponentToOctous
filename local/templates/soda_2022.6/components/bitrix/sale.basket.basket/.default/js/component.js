;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					let len = result.BASKET_DATA.BASKET_ITEM_RENDER_DATA.length;
					for (let i = 0; i < len; i++) {
						if (result.BASKET_DATA.BASKET_ITEM_RENDER_DATA[i]["SITE_ID"] === "s2") {
							result.BASKET_DATA.BASKET_ITEM_RENDER_DATA[i]["NAME"] = result.BASKET_DATA.BASKET_ITEM_RENDER_DATA[i]["NAME_EN"];
						}
					}
					...
					//console.log($(".basket-items-list-item-container").length);

					if($(".basket-items-list-item-container").length==0){
						$('.hide_empty_top').show();
						$('.hide_empty_bottom').hide();
					}else{
						$('.hide_empty_top').hide();
						$('.hide_empty_bottom').show();
					}
			...
			if (result.BASKET_ITEM_RENDER_DATA)
			{
				...
				for (i in result.BASKET_ITEM_RENDER_DATA)
				{
					if (result.BASKET_ITEM_RENDER_DATA.hasOwnProperty(i))
					{
						...
						{
							if(this.items[newData.ID].NOT_AVAILABLE !== newData.NOT_AVAILABLE){
								if(newData.NOT_AVAILABLE){
									let productName = newData.NAME;
									let color = '';
									let size = '';
									let message = BX.message('SALE_BASKET_NOT_AVAILABLE_POPUP');
									newData.PROPS.forEach(function(item){
										switch (item.CODE){
											case 'SIZES':
												size = ', '+item.VALUE;
												break;
									...
									message = message.replace('#PRODUCT_NAME#', productName+color+size);
									popupOpen('message', false, message);
								}
							}
		...
		createBasketItem: function(itemId)
		{
			...
			{
        if(this.items[itemId]['SITE_ID'] == 's2')
          this.items[itemId]['NAME'] = this.items[itemId]['NAME_EN'];
