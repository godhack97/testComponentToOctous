	...
	BX.Sale.BasketComponent = {
		...
		sendRequest: function(action, data)
		{
          ...
          SetOrder();
						...
						if (this.lastAction === 'refreshAjax')
						{
		...
		applyBasketResult: function(result)
		{
			...
			if (result.BASKET_ITEM_RENDER_DATA)
			{
				...
				//console.log(result.BASKET_ITEM_RENDER_DATA.length);
		...
		deleteAction: function()
		{
			...
			if (itemData)
			{
				...
				//delete BX.Sale.BasketComponent.items[31560];
				/*BX.Sale.BasketComponent.deleteBasketItem(itemData);
				BX.Sale.BasketComponent.shownItems = BX.Sale.BasketComponent.shownItems.filter(function(item) {
					return item !== itemData.ID
				})*/
				BX.Sale.BasketComponent.sortedItems = BX.Sale.BasketComponent.sortedItems.filter(function(item) {
					return item !== itemData.ID
				})
