...
$arSelectedFields[] = "PROPERTY_COLOR";
...
while($arRes = $rsData->GetNext())
{
    ...
    if($arRes["PROPERTY_COLOR_VALUE"]){


        $reslist=$PsuTaskControl::getList([
            'filter'=>[
                "UF_XML_ID"=>$arRes["PROPERTY_COLOR_VALUE"],
            ],
        ]);
        if ($el=$reslist->fetch())
        {
            $arRes["PROPERTY_COLOR_VALUE"]=$el["UF_NAME"];
        }
    }
	...
	$row->AddActions(array(
		array(
			...
			"ACTION"=>"javascript:SelEl('".CUtil::JSEscape($index)."', '".
                htmlspecialcharsbx(htmlspecialcharsbx(CUtil::JSEscape($arRes["~NAME"]." - ".$arRes["PROPERTY_COLOR_VALUE"]), ENT_QUOTES))."')",
		),
	));
}
